    ...
    public class ConverterHelper : IConverterHelper
    {
        ...
        public async Task<User> ToUserAsync(UserViewModel model, Guid imageId, bool isNew)
        {
            return new User
            {
                ...
                DocumentType = await _context.TypeDocuments.FindAsync(model.DocumentTypeId),
                Email = model.Email,
        ...
        public async Task<Vehicle> ToVehicleAsync(VehicleViewModel model, bool isNew)
        {
            return new Vehicle
            {
                ...
                VehicleType = await _context.VehiculeTypes.FindAsync(model.VehicleTypeId)
            };
        }
        ...
        public VehicleViewModel ToVehicleViewModel(Vehicle vehicle)
        {
            return new VehicleViewModel
            {
                ...
                BrandType = _combosHelper.GetComboBrands(),
                Color = vehicle.Color,
                Id = vehicle.Id,
                Line = vehicle.Line,
                Model = vehicle.Model,
